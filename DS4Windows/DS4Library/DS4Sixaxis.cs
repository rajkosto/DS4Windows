...
namespace DS4Windows
{
    ...
    public class DS4SixAxis
    {
        ...
        public void handleSixaxis(byte[] inputReport, DateTime receivedTimestamp, string MacAddress)
        {
            ...
            float angVelPitch   = (float)(intPitch) / 16;  //in deg/s
            float angVelYaw     = (float)(intYaw)   / 16;  //in deg/s
            float angVelRoll    = (float)(intRoll)  / 16;  //in deg/s

            float accelX        = (float)(intAccX) / 8192; //in Gs/s^2
            float accelY        = (float)(intAccY) / 8192; //in Gs/s^2
            float accelZ        = (float)(intAccZ) / 8192; //in Gs/s^2
